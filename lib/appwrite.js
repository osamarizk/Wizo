...
export async function saveDeviceToken(userId, deviceToken) {
  try {
    if (!userId || !deviceToken) {
      throw new Error("User ID and device token are required.");
    }

    // 1. Fetch the user's document using the user ID as the document ID
    ...
    // 2. Get the existing device tokens, or an empty array if none exist
    const existingTokens = userDoc.deviceToken || [];

    // 3. Create a new array, ensuring no duplicate tokens exist
    const updatedDeviceTokens = [...new Set([...existingTokens, deviceToken])];

    // 4. Update the user document with the new list of tokens
    ...
    console.log(`Device token saved for user ${userId}:`, deviceToken);
    return updatedUser;
  } catch (error) {
    console.error("Failed to save device token:", error);
    // You can handle specific errors here, e.g., 'document not found'
    throw new Error(
...
export async function removeDeviceToken(userId, deviceToken) {
  try {
    ...
    // 2. Filter out the specific device token
    const existingTokens = userDoc.deviceTokens || [];
    const updatedDeviceTokens = existingTokens.filter(
