...
export async function saveDeviceToken(userId, pushToken) {
  try {
    ...
    // 1. Fetch the current user document
    ...
    // 2. Prevent duplicates by checking if the token already exists.
    if (existingTokens.includes(pushToken)) {
      console.log("Push token already exists for this user, skipping save.");
      return userDoc;
    }

    // 3. Add the new push token to the array.
