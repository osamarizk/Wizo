...
export async function createReceipt(data) {
  try {
    const doc = await databases.createDocument(
      databaseId,
      "receipt_id", // 🔁 Replace this with your actual Receipt Collection ID
      ID.unique(),
      data
    );
    return doc;
...
export async function uploadReceiptImage(file) {
  try {
    const uploadedFile = await storage.createFile(
      storageId, // ✅ Your bucket ID
      ID.unique(),
      file // This should be a File object (React Native: use Blob or File from image picker)
    );
    return uploadedFile;
  } catch (error) {
