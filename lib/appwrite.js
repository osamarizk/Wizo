...
export const updateUserPremiumStatus = async (userId, isPremium) => {
  try {
    // Directly update the document using the userId as the document ID.
    // This is valid if your user profile document's $id is the same as the Appwrite account ID.
    const updatedUser = await databases.updateDocument(
      databaseId,
      userCollectionId,
      userId, // Use userId directly as the document ID
      {
        isPremium: isPremium,
        premiumStartDate: isPremium ? new Date().toISOString() : null, // Optional: record start date
        // You might also add premiumEndDate for fixed subscriptions, or for auto-renewable subscriptions
        // you'd track the renewal period or rely on payment gateway webhooks.
      }
    );
