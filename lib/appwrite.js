...
export async function sendOTPToEmail(email) {
  try {
    ...
    const userId = ID.unique();
    const tokenResponse = await account.createEmailSession(userId, email);
...
export const getMonthlyReceiptSummary = async (userId) => {
  try {
    const receipts = await databases.listDocuments(
      databaseId,
      receiptCollectionId,
      [Query.equal("user_id", userId), Query.orderDesc("datetime")] // Order by datetime to get recent first
    );

    console.log("Monthly receipts.....", receipts);
    const monthlyData = {};

    receipts.documents.forEach((receipt) => {
      const receiptDate = new Date(receipt.datetime);
      const year = receiptDate.getFullYear();
      // getMonth() is 0-indexed, so add 1 and pad with '0' for single-digit months
    ...
    // Convert the object to an array and sort by month (oldest first)
    ...
    // IMPORTANT FIX: Return an empty array on error instead of throwing
    // This ensures monthlySummary is always an array, preventing the TypeError.
    return [];
  }
};
...
export async function getUserEarnedBadges(userId) {
  try {
    // 1. Fetch user's earned badge records from userBadgesCollectionId
    // Ensure that your Appwrite query for userBadgesCollectionId might need to
    // include a .select() or .populate() if the 'badge' relationship isn't
    // automatically populating the full object. However, based on 'badge.$id',
    // it implies the 'badge' object is already accessible.
    ...
    // 2. For each earned badge record, fetch its details from the 'badges' collection
    for (const earnedBadgeDoc of earnedBadgesDocuments) {
      // Use earnedBadgeDoc.badge.$id to get the ID of the linked badge
      const badgeId = earnedBadgeDoc.badge_id?.$id; // Using optional chaining in case 'badge' object isn't present

      if (badgeId) {
        try {
          const badgeDetail = await databases.getDocument(
            databaseId,
            badgesCollectionId, // <-- Use the correct collection ID for your main 'badges' table
            badgeId
          );

          // Combine the earned badge's unique ID with the badge's details
          fullBadgeDetails.push({
            ...badgeDetail, // This includes name, description, icon_url, criteria, points
            $id: earnedBadgeDoc.$id, // Use the ID of the specific earned badge document (important for FlatList key)
            earnedAt: earnedBadgeDoc.$createdAt, // Assuming earnedBadgeDoc has $createdAt field, if not, adjust or remove
          });
        } catch (badgeError) {
          console.warn(
          ...
          // Optionally, push a partial or placeholder badge if details can't be fetched
          fullBadgeDetails.push({
            $id: earnedBadgeDoc.$id,
            name: "Unknown Badge",
            description: "Details not available.",
            icon_url: null, // or a placeholder icon
            points: 0,
          });
        }
      }
