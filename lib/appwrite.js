...
export async function fetchUserReceipts(userId) {
  try {
    const res = await databases.listDocuments(databaseId, receiptCollectionId, [
      Query.equal("user_id", userId),
    ]);
...
export async function getReceiptStats(userId) {
  try {
    ...
    const totalSpending = monthlyReceipts.documents.reduce((sum, doc) => {
      const total = parseFloat(doc.total);
      return sum + (isNaN(total) ? 0 : total);
    }, 0);

    // Get latest receipt (sorted by datetime descending)
