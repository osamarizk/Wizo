...
export async function saveDeviceToken(userId, pushToken, platform) {
  try {
    ...
    // 1. Create unique device ID
    const deviceId = ID.unique();

    // 2. Register device with Appwrite Messaging
    await messaging.createDevice(deviceId, platform, pushToken);
    console.log(`Device registered in Appwrite with ID: ${deviceId}`);

    // 3. Fetch current user document
    ...
    if (existingDeviceIds.includes(deviceId)) {
      console.log("Device ID already exists for this user, skipping save.");
      return userDoc;
    }

    // 5. Save deviceId (NOT pushToken) in user document
    const updatedDeviceIds = [...existingDeviceIds, deviceId];
    ...
    console.log(`Device ID saved for user ${userId}: ${deviceId}`);
    return updatedUser;
  } catch (error) {
