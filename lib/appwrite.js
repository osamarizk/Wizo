...
export async function createReceipt(data) {
  try {
    ...
    if (isCashPaymentMethod(doc.payment_method)) {
      console.log(isCashPaymentMethod(doc.payment_method));
      try {
        await addWalletTransaction(
          doc.user_id,
          parseFloat(doc.total),
          "manual_expense", // Treat as manual expense from receipt
          `Receipt: ${doc.merchant || "Unknown"} (${format(
            new Date(doc.datetime),
            "MMM dd,PPPP"
          )})`,
          doc.$id // Link to the new receipt's ID
        );
        console.log("Wallet transaction added for new cash receipt:", doc.$id);
      } catch (walletError) {
        console.warn(
...
export async function fetchBudget(budgetId) {
  ...
  try {
    const budget = await databases.getDocument(
      config.databaseId,
      config.budgetCollectionId, // Use your budget collection ID
      budgetId
    );
    return budget;
  } catch (error) {
