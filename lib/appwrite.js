...
export async function saveDeviceToken(userId, pushToken) {
  try {
    if (!userId || !pushToken) {
      throw new Error("User ID and push token are required.");
    }

    // 1. Fetch the user profile document using a query on the account ID.
    // This is more reliable as the document ID may not be the same as the user ID.
    const userProfileResponse = await databases.listDocuments(
      config.databaseId,
      config.userCollectionId, // Assuming this is the correct collection ID
      [Query.equal("accountid", userId)]
    );

    if (userProfileResponse.documents.length === 0) {
      console.error("User profile document not found. Cannot save token.");
      return;
    }
    const userProfileDoc = userProfileResponse.documents[0];

    // Get the existing list of device tokens from the user document.
    const existingTokens = userProfileDoc.deviceTokens || [];

    // 2. Validate that the new token is a string and is a valid Expo token.
    if (
      typeof pushToken !== "string" ||
      !pushToken.startsWith("ExponentPushToken")
    ) {
      console.warn("Invalid push token format received, skipping save.");
      return;
    }

    // 3. Filter out any existing tokens that are not the new token
    // to ensure we only have one token at a time.
    const updatedTokens = [pushToken];

    // 4. Add the new push token to the beginning of the array.
    ...
    throw new Error(
      `Failed to save push token: ${error.message || "Unknown error"}`
    );
  }
}
// This function now removes the Appwrite-generated device ID from the user's document
export async function removeDeviceToken(userId, pushToken) {
  try {
    if (!userId || !pushToken) {
      throw new Error("User ID and push token are required.");
    }
    ...
    // 2. Remove the token from the array.
    ...
    throw new Error(
      `Failed to remove push token: ${error.message || "Unknown error"}`
    );
  }
}
...
export const sendPushNotification = async (to, title, body, data = {}) => {
  try {
    // The payload for the Appwrite function
    const payload = JSON.stringify({ to, title, body, data });

    console.log("Starting asynchronous Appwrite function execution...");

    // Execute the Appwrite function asynchronously (wait: true)
    // We do not await this, as the app should not wait for a response.
    // The response object here will only confirm the initiation.
    await functions.createExecution(
      "68a7e13f0032fcb68a19", // Your Appwrite Function ID
      payload,
      true // Set this to true for asynchronous execution
    );

    console.log("Appwrite function execution initiated successfully!");
    // With asynchronous execution, we can't get the function's response.
    // So we assume success from the client side.
    // Alert.alert(
    //   "Success",
    //   "Notification process started. Check Appwrite logs for details."
    // );
  } catch (error) {
    // This catch block will only be triggered if the request to start the function fails,
    // not if the function itself has an error.
    console.error("Error initiating Appwrite function execution:", error);
    Alert.alert(
      "Push Notification Error",
      `An error occurred while starting the notification process: ${error.message}`
    );
  }
};
...
export const getFinancialSummary = async (userId, currentLanguage) => {
  if (!userId) {
    throw new Error("User ID is required to fetch financial data.");
  }
  ...
  try {
    ...
    const receiptsResponse = await databases.listDocuments(
      databaseId,
      receiptCollectionId,
      [
        Query.equal("user_id", userId),
        Query.greaterThan("datetime", thirtyDaysAgo.toISOString()),
      ]
    );
    const allReceipts = receiptsResponse.documents;
    ...
    const userBudgets = budgetsResponse.documents;
    ...
    allReceipts.forEach((receipt) => {
      ...
      if (parsedItems.length > 0) {
        parsedItems.forEach((item) => {
          ...
          if (categoryId && !isNaN(itemPrice) && itemPrice > 0) {
            ...
            if (!categoryIdToNameMap[categoryId] && item.category) {
      ...
      // Logic for recent large expenses, simplified to just check the amount
      if (totalReceiptAmount > 500) {
        const mainReceiptCategory = parsedItems[0]?.category || "Uncategorized";
        const receiptDate = new Date(receipt.datetime);
        recentLargeExpenses.push({
    ...
    const topSpendingCategories = Object.entries(categorySpendingById)
      .map(([id, amount]) => ({
    ...
    const topFrequentMerchants = Object.entries(merchantVisits)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 3)
      .map(([name, count]) => ({ name, count }));

    const topFrequentItems = Object.entries(itemOccurrences)
      .filter(([, count]) => count > 1)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 3)
      .map(([name, count]) => ({ name, count }));

    const budgetPerformance = userBudgets
      .map((budget) => {
    ...
    const totalWalletBalance = walletResponse.documents.reduce(
      (sum, transaction) => {
        if (
...
export async function createReceipt(data, userId, currentReceiptCount) {
  ...
  try {
    receiptDoc = await databases.createDocument(
      databaseId,
      receiptCollectionId,
      ID.unique(),
      data
    ...
    if (isCashPaymentMethod(receiptDoc.payment_method)) {
      ...
      try {
        // --- THE FIX IS HERE ---
        // Changed "MMM dd,PPPP" to "MMM dd, yyyy" to avoid locale-specific format errors.
        ...
        await addWalletTransaction(
          ...
          `Receipt: ${receiptDoc.merchant || "Unknown"} (${formattedDate})`,
    ...
    if (userId) {
      ...
      try {
        updatedUserDoc = await databases.updateDocument(
          databaseId,
          userCollectionId,
