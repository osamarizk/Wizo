...
export const sendPushNotification = async (to, title, body, data = {}) => {
  try {
    ...
    const response = await functions.createExecution(
      "68a7e13f0032fcb68a19", // Replace with your Appwrite Function ID
      payload,
      false // Set this to false for synchronous execution
    );

    const result = JSON.parse(response.responseBody);
    console.log("Appwrite Function Execution Result:", result);

    if (result.success) {
      Alert.alert("Success", "Push notification sent successfully!");
    } else {
      Alert.alert("Push Notification Error", result.message);
    }
  } catch (error) {
    console.error("Error executing Appwrite function:", error);
    Alert.alert(
